...
const SupplierManager: React.FC<SupplierManagerProps> = ({ suppliers, onAddSupplier, onUpdateSupplier, onDeleteSupplier }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingSupplier) {
        const updated: Supplier = { ...editingSupplier, ...formData } as Supplier;
        success = await onUpdateSupplier(updated);
      } else {
        const newSupplier: Supplier = {
          ...formData as Supplier,
          id: crypto.randomUUID(), // Génération d’un id unique ici
        };
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Êtes-vous sûr de vouloir supprimer ce fournisseur ?')) {
      try {
        const success = await onDeleteSupplier(id);
        if (!success) alert("Impossible de supprimer le fournisseur. Vérifiez les permissions ou la connexion.");
      } catch (e) {
        console.error(e);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div key={supplier.id} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 group hover:border-indigo-300 transition-all">
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => handleEdit(supplier)} className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-full" aria-label={`Modifier ${supplier.name}`}>
                  <Edit2 size={16} />
                </button>
                <button onClick={() => handleDelete(supplier.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-full" aria-label={`Supprimer ${supplier.name}`}>
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
        ...
        {filteredSuppliers.length === 0 && (
          <div className="col-span-full py-12 text-center text-slate-gray-400 bg-white rounded-xl border border-dashed border-slate-300">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-600" aria-label="Fermer le formulaire">
                <X size={24} />
              </button>
            </div>

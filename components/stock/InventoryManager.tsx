...
const InventoryManager: React.FC<InventoryManagerProps> = ({ products, suppliers, onUpdateProduct, onAddProduct, onDeleteProduct }) => {
  ...
  const handleSave = async () => {
    ...
    const productToSave: Product = {
      ...
      imageUrl: formData.imageUrl || `https://picsum.photos/seed/${Date.now()}/300/300`,
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      ...
      alert("Erreur génération description IA");
    } finally {
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm divide-y divide-slate-100">
              ...
              {filteredProducts.map(product => {
                ...
                const marginPercent = product.price ? (margin / product.price) * 100 : 0;
